...
<div class="_panel" :class="[$style.root, { [$style.compact]: compact }]">
	...
	<div :class="$style.appInfo">
		<div :class="$style.appName">{{ activeResolved.manifest.name }}</div>
		<div :class="$style.publisher">{{ i18n.tsx._miniApps.publishedBy({ name: activeResolved.manifest.publisher.name }) }}</div>
		<div :class="$style.origin">{{ appHostname }}</div>
		<div v-if="!compact" :class="$style.notice">{{ i18n.ts._miniApps.externalNotice }}</div>
	</div>
	<MkButton :class="$style.openButton" primary small inline @click="open">
	...
	<MkModalWindow
		...
		:width="dialogWidth"
		:height="dialogHeight"
		:withCloseButton="pendingAuthorization == null"
		@close="close"
		@closed="onClosed"
		@click="close"
		@esc="onDialogEsc"
...
import { computed, nextTick, onDeactivated, onUnmounted, ref, shallowRef, useTemplateRef, watch } from 'vue';
...
const activeResolved = shallowRef(props.resolved);
const pendingAuthorization = shallowRef<PendingAuthorization | null>(null);
const appHostname = computed(() => new URL(activeResolved.value.appOrigin).hostname);
const dialogWidth = Math.min(960, Math.max(320, window.innerWidth - 32));
const dialogHeight = Math.min(760, Math.max(360, window.innerHeight - 32));
...
async function open(): Promise<void> {
	...
	activeResolved.value = props.resolved;
	opened.value = true;
...
function onDialogEsc(): void {
	if (pendingAuthorization.value != null) {
...
async function prepareLaunch(sequence: number): Promise<void> {
	...
	if (!Number.isFinite(expiresAt) || expiresAt <= Date.now()) {
		...
		activeResolved.value = refreshed;
	}
...
function focusFirstAuthorizationControl(): void {
	const firstFocusable = getAuthorizationFocusableElements()[0];
